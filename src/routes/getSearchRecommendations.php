<?php
$app->post('/api/Europeana/getSearchRecommendations', function ($request, $response, $args) {
    $settings = $this->settings;

    //checking properly formed json
    $checkRequest = $this->validation;
    $validateRes = $checkRequest->validate($request, ['query']);
    ...
    $query_str = $settings['api_url'] . "v2/suggestions.json";
    $body = array();
    ...
    if (isset($post_data['args']['phrases']) && strlen($post_data['args']['phrases']) > 0) {
        $body['phrases'] = $post_data['args']['phrases'];
    }
